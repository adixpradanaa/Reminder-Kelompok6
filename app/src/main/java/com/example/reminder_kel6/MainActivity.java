package com.example.reminder_kel6;
...
public class MainActivity extends AppCompatActivity {
    ...
    private void insertDataToDb(String title, String date, String time) {
        boolean insertData = database.insertData(title, date, time);
        if (insertData) {
            try {
                populateListView();
    ...
    private void populateListView() {
        try {
            ArrayList<Data> items = database.getAllData();
            Item itemsAdopter = new Item(this, items);
